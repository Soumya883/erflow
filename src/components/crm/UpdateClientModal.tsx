...
import { updateClient, deleteClient } from "@/app/actions/crm";

export function UpdateClientModal({ client }: { client: any }) {
      ...
      <DialogContent className="sm:max-w-[425px] p-0 overflow-hidden border-none shadow-2xl rounded-3xl">
        <div className="px-6 pt-6 pb-4 bg-primary/5 flex justify-between items-start">
          <DialogHeader className="w-full">
            ...
            <DialogTitle className="text-center text-2xl font-bold">Edit Client</DialogTitle>
          </DialogHeader>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input name="name" required defaultValue={client.name} className="rounded-xl" />
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Company</label>
              <Input name="company" required defaultValue={client.company} className="rounded-xl" />
            </div>
          </div>

          <div className="space-y-2">
            <label className="text-sm font-medium">Email</label>
            <Input name="email" type="email" required defaultValue={client.email} className="rounded-xl" />
          </div>

          <div className="space-y-2">
            <label className="text-sm font-medium">Phone (Optional)</label>
            <Input name="phone" defaultValue={client.phone || ""} className="rounded-xl" />
          </div>
          
          <div className="space-y-2">
            <label className="text-sm font-medium">Billing Address (Optional)</label>
            <Textarea name="address" defaultValue={client.address || ""} className="rounded-xl" />
          </div>
