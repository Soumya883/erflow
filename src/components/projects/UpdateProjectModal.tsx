...
import { Loader2, Edit, FolderKanban } from "lucide-react";
import { toast } from "sonner";
import { updateProject, deleteProject } from "@/app/actions/work";

export function UpdateProjectModal({ project }: { project: any }) {
      ...
      <DialogContent className="sm:max-w-[425px] p-0 overflow-hidden border-none shadow-2xl rounded-3xl">
        <div className="px-6 pt-6 pb-4 bg-primary/5 flex justify-between items-start">
          <DialogHeader className="w-full">
            ...
            <DialogTitle className="text-center text-2xl font-bold">Edit Project</DialogTitle>
          </DialogHeader>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4 space-y-4">
          <div className="space-y-2">
            ...
            <Input name="name" required defaultValue={project.name} className="rounded-xl" />
          </div>
          
          <div className="space-y-2">
            <label className="text-sm font-medium">Description</label>
            <Textarea name="description" defaultValue={project.description || ""} className="rounded-xl" />
          </div>
